...
def singleTAR(tickerAsArray, progress):
    ...
    #Function Globals
    try: 
        open, high, low, close, volume = getHistory(tickerAsArray)
        lenClose = len(close)
    ...
    try:
        ...
        sma50 = closePrice / pd.Series(MA(tickerAsArray)[0]).values[0] - 1
        sma100 = closePrice / pd.Series(MA(tickerAsArray, 100)[0]).values[0] - 1
        sma200 = closePrice / pd.Series(MA(tickerAsArray, 200)[0]).values[0] - 1
